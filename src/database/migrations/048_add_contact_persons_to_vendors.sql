-- Migration: Change vendors to use contact_persons array like brokers/leads
-- This adds contact_persons JSONB column and migrates existing data

-- 1. Add contact_persons column (with default empty array)
ALTER TABLE vendors ADD COLUMN IF NOT EXISTS contact_persons JSONB DEFAULT '[]';
...
ALTER TABLE vendors ALTER COLUMN contact_persons SET NOT NULL;

-- 4. Create GIN index for contact_persons searches
CREATE INDEX IF NOT EXISTS idx_vendors_contact_persons ON vendors USING GIN (contact_persons);

-- 5. Add comment
COMMENT ON COLUMN vendors.contact_persons IS 'Array of contact persons with name, phones[], and emails[] - matches broker/lead structure';
